# Subtitle Studio Plus について

## これは何をするアプリ？
- 音声や動画ファイルを読み込みます。
- AIで字幕を作ります。
- 波形を見ながら字幕の時間を直します。
- 最後に `.srt` という字幕ファイルを書き出します。

## 全体の作り
- `SwiftUI`
  - 画面を作る部品です。
  - 例: ボタンや一覧を組み立てる土台です。
- `AVFoundation`
  - 音を読む仕組みです。
  - 例: 音楽プレーヤーの心臓部分です。
- `Gemini API`
  - AIに音声を送って文字にしてもらいます。
  - 例: 音を聞いてメモを作る外部スタッフです。
  - 今は `streamGenerateContent` で少しずつ返事を受け取ります。
  - そのため、生成中の進み具合を画面へ出せます。
- `Keychain`
  - APIキーを安全に保管します。
  - 例: Macの中にある鍵付きの引き出しです。

## フォルダの意味
- `Sources/App`
  - アプリの入口です。
- `Sources/Models`
  - 字幕や音声の型です。
- `Sources/ViewModels`
  - 画面の状態をまとめる司令塔です。
  - 生成中の進捗もここで調整します。
- `Sources/Views`
  - 実際に見えるUIです。
- `Sources/Services`
  - 音声解析、波形生成、AI通信などの仕事役です。
- `Sources/Persistence`
  - 保存まわりです。
- `Sources/Utilities`
  - 時刻変換やSRT処理の小道具です。

## なぜこの技術を選んだ？
- SwiftUI
  - macOS向けに自然に作れるからです。
- AVFoundation
  - Apple標準で音声処理に強いからです。
- 依存を増やさない方針
  - 壊れる場所を減らせるからです。

## よくあるバグと直し方
- APIキー未設定
  - 症状: 字幕生成で止まります。
  - 修正: Settingsでキーを入れて保存します。
- 対応外ファイル
  - 症状: 読み込み時に失敗します。
  - 修正: 対応拡張子の音声へ変換します。
- 長い音声で遅い
  - 症状: AI解析に時間がかかります。
  - 修正: 300秒ごとに分割して送る仕組みを確認します。
- 進捗バーが不自然
  - 症状: 数値とバーが合わない、途中で止まって見えます。
  - 修正: `AnalysisProgress` の `actualPercent` と `displayPercent` を確認します。
- 波形と字幕が少しずれる
  - 症状: タイミングが気持ち悪いです。
  - 修正: 自動補正を実行し、必要なら手で微調整します。

## 落とし穴
- 画面の見た目を真似しすぎると、Macらしさが消えます。
- 逆にMacらしさを強くしすぎると、元UIと違って見えます。
- 音声を丸ごとメモリに読むので、大きいファイルは重くなります。
- Geminiの返事は少しずつ届くので、SSEの途中イベントが空でも異常とは限りません。
- 進捗表示は `受信した量` を元にしているので、Gemini内部の本当の処理率とは一致しません。

## ベストプラクティス
- 字幕を変える前に Undo 用の履歴を積む。
- 失敗はダイアログで止めて分かりやすくする。
- SRTの時刻変換は必ずテストする。
- AIの返答はそのまま信用せず、SRTとして解析してから使う。
- 生成中UIは、文字列1本ではなく進捗用の型で管理する。
- ストリーミング通信は、途中の小さい返事を連結して最後にSRTとして使う。

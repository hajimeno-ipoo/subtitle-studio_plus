...
- `Local Pipeline`
  ...
  - 歌詞参照がない時は、まず本文転写を 1 回だけ走らせて、その時間情報を `timing guide` にも使います。時間が足りない時だけ補助の再転写を足します。
- `Keychain`
  - API キーを安全に保存します。
- `Resolve 連携`
...
## Local Pipeline の考え方
- まず `whisper.cpp C API` を「自然な歌詞本文」を作る役として使います。
- 歌詞参照がない時は、その本文転写の時間情報も `timing guide` に流用します。
- ただし時間情報が足りない時だけ、`timing guide` 専用の転写を追加で走らせます。
